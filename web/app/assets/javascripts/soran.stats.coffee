...
Stats.prototype =
  ...
  draw: (k) ->
    ...
    soran.withCircle $stats, this, this.getData, [], (d) ->
      ...
      switch k
        when 'day'
        then that.drawLinear(soran.fn.groupBy(d.content, that.keyDay))
        when 'punchcard'
        then that.drawPunch(soran.fn.groupBy(d.content, that.keyDay))
  ...
  sortPunch: (grouped, callback) ->
    ...
    mapValueFunc = (v) ->
      g = (x) ->
        d = new Date(x.createdAt)
        d.getHours().toString()
      a = soran.fn.groupBy v, g
      soran.fn.mapValue a, (x) ->
        x.length
    d = soran.fn.mapValue(grouped, mapValueFunc)
    soran.fn.map m
    m = (k, v) ->
      d =
        day: k
    ...
    data = soran.fn.map d, m
    a = []
    currD = data.map (d, i) ->
      d.day
    currD = currD.map (d, i) ->
      parseInt d
    dif = soran.fn.diff day7, currD
    for x in dif
      d =
        day: x
        hours: {}
    ...
    data = data.map (d, i) ->
      ...
      currA = currA.map (d, i) ->
        parseInt d
      dif = soran.fn.diff hours24, currA
      for x in dif
  ...
  drawLinear: (grouped) ->
    ...
    grouped = soran.fn.mapValue grouped, mapValueFunc
    mapFunc = (k, v) ->
      d =
        kind: k
        value: v
    ...
    data = soran.fn.map grouped, mapFunc
    listenDays = data.map (d, i) ->
      d.kind

    diff = soran.fn.diff ["0", "1", "2", "3", "4", "5", "6"], listenDays
...
if $stats.length isnt 0
  ...
  soran.stats.identifier = identifier
  soran.stats.draw('day')
  tabActive = ($elem) ->
  ...
  $('.stat-decision li a').on 'click', (e) ->
    $('#stats svg').remove()
    tabActive $(this)
    soran.stats.draw($(this).data('kind'))
