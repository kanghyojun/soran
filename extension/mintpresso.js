...
(function() {
  var __init__, _accId, _addEdge, _addPoint, _callbackEnabled, _callbackName, _dataType, _edge_proto, _findRelations, _getPoint, _getPointByTypeOrIdentifier, _initial, _initialized, _key, _log, _logPrefix, _mark, _pageTracker, _point_proto, _serverIteration, _servers, _timeout, _urls, _verbs, _versionPrefix;

  __init__ = function(d, key) {
    var entry, script, server;
    ...
    return entry.parentNode.insertBefore(script, entry);
  };
  ...
  String.format = function() {
    var i, reg, s, _i, _ref;
    s = arguments[0];
    for (i = _i = _ref = arguments.length - 1; _i <= 0; i = _i += 1) {
      reg = new RegExp("\\{" + i + "\\}", "gm");
  ...
  try {
    ...
    _getPoint = function(id, callback) {
      var retry;
      retry = function() {
    ...
    _getPointByTypeOrIdentifier = function(json, callback, option) {
      var i, key, retry, _identifier, _type;
      retry = function() {
        if (_serverIteration === _servers.length - 1) {
          _serverIteration = 0;
          return _log(function() {
      ...
      for (key in json) {
        ...
        if (key.length === 0 || key !== _mark) {
          _type = encodeURIComponent(key);
        }
        if (json[key].length === 0 || json[key] === _mark) {
    ...
    _findRelations = function(json, callback, option) {
      var i, key, oId, oType, retry, sId, sType, v;
      console.log("edge,");
      console.log(json);
      console.log(_mark);
      retry = function() {
      ...
      for (key in json) {
        switch (i) {
          case 1:
            console.log('subject,', key, json[key]);
            sType = encodeURIComponent(key);
            if (json[key] !== _mark && typeof json[key] === 'number') {
              sId = encodeURIComponent(json[key]);
            }
            break;
          case 2:
            if (_verbs.indexOf(key) === -1) {
    ...
    _addPoint = function(json, callback, update) {
      var key, retry, value;
      retry = function() {
        if (_serverIteration === _servers.length - 1) {
          _serverIteration = 0;
    ...
    _addEdge = function(json, callback, update) {
      var i, key, retry, value;
      retry = function() {
        if (_serverIteration === _servers.length - 1) {
          _serverIteration = 0;
          return _log(function() {
      ...
      for (key in json) {
        switch (i) {
          case 1:
            if (key !== _mark && _edge_proto.indexOf(key) === -1) {
      ...
      return jQuery.ajax({
        ...
        error: function(xhr, status, error) {
          if(error == 'timeout')
            console.log('Timeout error occured, for more information see below')
            console.log('xhr,', xhr)
    ...
    window.mintpresso = {
      ...
      get: function() {
        ...
        */
        ...
        var callback, conditions, hasMark, json, key, option;
        if (_initialized === false) {
          return console.warn(_logPrefix + 'Not initialized. Add mintpress.init in your code with API key.');
        }
        if (arguments.length === 0) {
          return console.warn(_logPrefix + 'An argument is required for mintpresso.get method.');
          ...
          if (typeof arguments[0] === 'number') {
            ...
            if ((hasMark && conditions > 1) || conditions === 3) {
              return _findRelations(arguments[0], callback, option);
            } else {
              return _getPointByTypeOrIdentifier(arguments[0], callback, option);
            }
          } else {
            return console.warn(_logPrefix + 'An argument type of Number or String is required for mintresso.get method.');
          }
        } else {
          return console.warn(_logPrefix + 'Too many arguments in mintpresso.get method.');
        }
      },
      trackPage: function() {
        return true;
      ...
      set: function() {
        ...
        */
        ...
        var callback, isEdgeOperation, key, option;
        if (_initialized === false) {
          return console.warn(_logPrefix + 'Not initialized. Add mintpress.init in your code with API key.');
        }
        if (arguments.length === 0) {
          ...
          if (typeof arguments[0] === 'object') {
            ...
            if (arguments[1] !== void 0 && typeof arguments[1] === 'function') {
              callback = arguments[1];
              option = arguments[2];
            } else if (arguments[1] !== void 0 && typeof arguments[1] === 'boolean') {
              callback = mintpresso.callback;
              option = arguments[1];
            } else {
              callback = mintpresso.callback;
      ...
      callback: function(response) {
        return console.log(_logPrefix + ("Response(" + response.status.code + "): "), response);
      }
    };
  } catch (e) {
    if (window['__mintpresso__'] !== void 0 && true) {
      throw e;
    } else {
